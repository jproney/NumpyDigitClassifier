...
def gradient_descent(train_ex, solutions, grad_fn, alpha=.0001, iterations=1000, track_err = False):
    if track_err:
...
def sgd_optimize(train_ex, solutions, grad_fn, mini_batch_size, alpha = .0001, iterations = 1000, track_err = False):
    if track_err:
        err = np.zeros(iterations)
    curr_theta = np.zeros((train_ex.shape[1],))
    for i in range(iterations):
...
def adam_optimize(train_ex, solutions, grad_fn, mini_batch_size, alpha = .0001, gamma = .01, iterations = 1000, track_err = False):
    if track_err:
        err = np.zeros(iterations)
    curr_theta = np.zeros((train_ex.shape[1],))
