...
def gradient_descent(train_ex, solutions, grad_fn, alpha=.5, epochs=100, track_err = False, error_fn = None,init_theta = None,track_progress = False, num_logs= 100):
    if track_err:
...
def sgd_optimize(train_ex, solutions, grad_fn, mini_batch_size, alpha = .5, epochs = 100, track_err = False, error_fn = None, init_theta = None,track_progress = False,num_logs = 100):
...
def adam_optimize(train_ex, solutions, grad_fn, mini_batch_size, alpha = .5, gamma = .99, lamb = 0, w = 200, epochs = 100, track_err = False,error_fn = None, init_theta = None, track_progress = False, num_logs = 100):
    if track_err:
        err = np.zeros(num_logs) 
    if init_theta is None:
        init_theta = np.zeros((train_ex.shape[1],1))
    ...
    for i in range(epochs):
        ...
        for j in range(batches_per_epoch):
            curr_batch = min_batches[j*mini_batch_size:(j+1)*mini_batch_size,:] 
            curr_sol = sol_batches[j*mini_batch_size:(j+1)*mini_batch_size,:]
            curr_grad = grad_fn(curr_batch, curr_sol, curr_theta - gamma*moment) #add momentum before computing grad 
