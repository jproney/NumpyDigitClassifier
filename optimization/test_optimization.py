...
class LinearTestCase(unittest.TestCase):
    ...
    def test_basic_GD(self):
        ...
        (theta, err) = gd.gradient_descent(train_ex, sol, gd.squared_error_cost_gradient, epochs=200, track_err=True,
                                           error_fn=gd.squared_error_cost)
    ...
    def test_SGD(self):
        ...
        (train_ex, (in_add, in_scale)) = gd.column_normalize(train_ex)
        (sol, (out_add, out_scale)) = gd.column_normalize(sol)
        (theta, err) = gd.sgd_optimize(train_ex, sol, gd.squared_error_cost_gradient, 32, epochs=100, track_err=True,
                                       error_fn=gd.squared_error_cost)
        print("Converged in {} Seconds".format(time.time() - start))
        train_ex.sort(0)
        h = train_ex @ theta
    ...
    def test_adam(self):
        ...
        (train_ex, (in_add, in_scale)) = gd.column_normalize(train_ex)
        (sol, (out_add, out_scale)) = gd.column_normalize(sol)
        (theta, err) = gd.adam_optimize(train_ex, sol, gd.squared_error_cost_gradient, 32, epochs=100, track_err=True,
                                        error_fn=gd.squared_error_cost)
        print("Converged in {} Seconds".format(time.time() - start))
        train_ex.sort(0)
        h = train_ex @ theta
    ...
    def test_logistic(self):
        ...
        (theta, err) = gd.sgd_optimize(train_ex, sol, gd.cross_entropy_cost_gradient, 1, epochs=10, alpha=.001,
                                       init_theta=theta0, track_err=True, error_fn=gd.classification_accuracy,
                                       track_progress=True)
        plt.plot(np.arange(err.shape[0]), err)
        print(np.min(err))
        plt.show()
